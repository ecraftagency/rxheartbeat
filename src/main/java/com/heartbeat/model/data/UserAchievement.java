...
public class UserAchievement extends Achievement {
  ...
  public String claimAchievement(Session session, int achievementType, int milestoneId) {
    ...
    if (subMap == null)
      return Msg.map.getOrDefault(Msg.DTO_DATA_NOT_FOUND, "achievement_not_found");
    ...
    if (currentVal < dto.milestoneValue)
      return Msg.map.getOrDefault(Msg.INSUFFICIENT_CLAIM, "insufficient_record_count");

    if (checkClaim(achievementType, milestoneId))
      return Msg.map.getOrDefault(Msg.ALREADY_CLAIM, "milestone_already_claim");
